...
class BlackHole(GameObject):
    def __init__(self, force=2) -> None:
        super().__init__()
...
class Field:
    ...
    def get_hole_range(self, hole) -> list:
        range = []
    ...
    def check_holes(self):
        for id, hole in enumerate(self.holes, 0):
            self.holes_coordinates[id] = self.get_hole_range(hole)
